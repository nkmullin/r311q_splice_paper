...
###############################################  
  ...
  selection_method = as.character(my_filter_csv[,4])
  variable_genes = my_filter_csv[,5]
  
  for(i in 1:length(my_filepaths)){
    my_initial_object <- my_list[[i]]
    my_intial_object <- subset(my_initial_object, subset = nFeature_RNA < higher.cutoff & nFeature_RNA > lower.cutoff & mito.genes < mito.cutoff)
